...
def projection_layer(input_size, hidden_size):
...
class ContextAttentionLayer(nn.Module):
    ...
    def forward(
        ...
        reference_attention = self.reference_projection(reference) # bs x ref_seq_length x attention_size
        context_attention = self.context_hidden_projection( 
            self.context_projection(context).permute(0, 2, 1)      # bs x ref_seq_length x attention_size
        ).permute(0, 2, 1)
        alphas = self.alpha_projection(
            torch.tanh(reference_attention + context_attention)    # bs x ref_seq_length => 각 reference 파트가 context와 얼마나 관련이 있는지.
        )  

        output = reference * torch.unsqueeze(alphas, -1)   # (bs x ref_seq_length x ref_hidden_size) X (bs x ref_seq_length x 1) = (bs x ref_seq_length x ref_hidden_size)
        output = torch.sum(output, 1) if average_seq else torch.squeeze(output) # (bs x ref_seq_length x ref_hidden_size)

        return output, alphas #  output = (bs , ref_hidden)


class ContextAttentionLayer_V2(nn.Module):
    """
    Implements context attention as in the PaccMann paper (Figure 2C) in
    Molecular Pharmaceutics.
    With the additional option of having a hidden size in the context.
    ...
    def __init__(
            ...
            individual_nonlinearity: type = nn.Sequential(),
            temperature: float = 1.0,
            SMILES_Omic_Flag: bool = True  # True: SMILES; False: Omic
    ):
        """Constructor
        Arguments:
            reference_hidden_size (int): Hidden size of the reference input
        ...
        self.individual_nonlinearity = individual_nonlinearity
        self.temperature = temperature
        self.SMILES_Omic_Flag = SMILES_Omic_Flag
        ...
        if context_sequence_length > 1:
                    ...
                    OrderedDict(
                        [
                            (
                                'projection',
                                nn.Linear(
                                    context_sequence_length,  # 默认 1 ; 多组学数据设置为 3
                                    reference_sequence_length,  # smiles 512
                                ),
                            ),
                            ('act_fn', individual_nonlinearity),
                        ]
            ...
            # self.context_hidden_projection = nn.Sequential()
            self.context_hidden_projection = nn.BatchNorm1d(attention_size
                                                            )
        self.alpha_projection = nn.Sequential(
            OrderedDict(
                [
    ...
    def forward(
            self,
            reference: torch.Tensor,  # encoded_smiles: 原始smiles Embedding + 三个尺度卷积smiles
            context: torch.Tensor,  # context : rna/mut/cnv
            average_seq: bool = True
    ):
        """
        Forward pass through a context attention layer
        ...
        assert len(reference.shape) == 3, 'Reference tensor needs to be 3D'
        assert len(context.shape) == 3, 'Context tensor needs to be 3D'
        # reference: bs 512 16(smiles embedding size)->64; context: bs 1 1654->64
        # 一个药物SMILES Vector(Embedding)对应一个细胞系的组学数据
        reference_attention = self.reference_projection(reference)
        if self.SMILES_Omic_Flag:
            context_attention = self.context_hidden_projection(
                self.context_projection(context).permute(0, 2, 1).squeeze(-1)
            ).unsqueeze(-1).permute(0, 2, 1)
        else:
            context_attention = self.context_hidden_projection(
